...
class SessionScreen(MDScreen):
    ...
    def __init__(self, workout_info: mapping.WorkoutOptionInfo = None, **kwargs):
        ...
        self.workout_info = option
        self.name: str = option.id #For registration with ScreenManager
...
class RecordLayout(ScrollView):
    ...
    def __init__(self, lift, sets, **kwargs):
        super().__init__(**kwargs)
        self.lift = lift
        self.set_initial_records()

    def set_initial_records(self):
...
class InputLayout(StackLayout):
    ...
    def log_lift(self):
        ...
        db.register_completed_lift(self.lift, sets)
        db.register_graph_data(self.lift, max)
        
        #This is hideous
        session_screen = self.parent.parent.parent.parent.parent.parent
        
        if not session_screen.session_registered:
            db.register_session()
            db.update_last_completed(session_screen.name)
            session_screen.session_registered = True

        record_layout = self.parent.parent.record_layout
        record_layout.stack.clear_widgets()
        record_layout.set_initial_records()
...
class InputRow(MDBoxLayout):
    ...
    def get_dict(self):
        try:
            return {'reps': int(self.reps), 'weight': int(self.weight)}
        except ValueError:
            return None
